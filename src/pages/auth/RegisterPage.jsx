...
export function RegisterPage() {
  const { register: registerAccount, loginWithGoogle, isRegistering, isAuthenticated } = useAuth();
    ...
    <div className="grid min-h-screen place-items-center bg-slate-50 p-6 dark:bg-slate-950">
      <div className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-6 shadow-soft dark:border-slate-800 dark:bg-slate-900">
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-black text-slate-950 dark:text-white">Register</h1>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Registration follows the mobile backend flow.
          </p>
        </div>
        ...
        <form className="grid gap-4" onSubmit={handleSubmit(submit)}>
          ...
          <Field label="Confirm password" error={errors.passwordConfirmation?.message}>
            <Input type="password" autoComplete="new-password" {...register('passwordConfirmation')} />
          </Field>
        ...
        <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
          ...
          <Link className="font-bold text-primary-light hover:text-primary" to={ROUTES.login}>
            Login
          </Link>
        </p>
      </div>
